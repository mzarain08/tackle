...
<div ppt-box  class="hide-mobile rounded mb-4 _sfavs search-zoom <?php if(isset($post->isFavs) && $post->isFavs){ ?>isFavs<?php } ?>" data-pid='%postid%' <?php if(isset($post->carddata)){ echo $post->carddata; } ?>>
  ...
  <div class="_content border-top pb-2">
    <?php if(defined('WLT_DEMOMODE') || !user_can($post->post_author, 'administrator')){ ?>
    <div class="position-absolute mt-n3 z-1" style="right:10px;">
      <?php echo $CORE_UI->AVATAR("user", array("size" => "md", "uid" => $post->post_author, "css" => "rounded-circle border bg-white", "online" => 0))  ?>
    </div>
    <?php } ?>
    ...
    <div class="d-flex align-items-baseline">
      <span class="fs-6 text-600">%hits% <?php echo __("views","premiumpress"); ?></span>
