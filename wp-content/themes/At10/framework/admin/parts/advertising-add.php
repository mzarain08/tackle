        ...
        <div class="form-group col-6">
          ...
          <div class="input-group mt-2">
            <select name="campaign[location]" class="form-control" >
			  ...
			  $kl = "";
			  if(isset($_GET['eid']) ){
			  $kl = get_post_meta($_GET['eid'], "location", true);
			  }
			  
			  foreach($CORE->ADVERTISING("get_spaces", array("enabled") ) as $key => $ban){ ?>
              <option value="<?php echo $key; ?>" <?php if(  $kl == $key){ echo "selected=selected"; } ?>><?php echo $ban['n']; ?> (<?php echo $ban['sw']; ?>x<?php echo $ban['sh']; ?>px)</option>
              <?php } ?>
            </select>
          </div>
        </div>
        ...
        <div class="col-md-6">
          <div class="form-group ">
            ...
            <div class="input-group date mt-2 position-relative"  id="expiry-date">
              <input name="campaign[expires]" class="form-control ppt-datepicker" data-target="#expiry-date"  type="text" value="<?php if(isset($_GET['eid']) && is_numeric($_GET['eid']) ){ echo get_post_meta($_GET['eid'], "expires", true); }?>">
