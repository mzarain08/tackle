...
<?php } ?>
<div id="MAINCONTAINER" class="add-listing-form mobile-negative-margin-x">
<?php
...
<div class="bg-white p-4 border  text-center col-md-8 mx-auto my-5">
  ...
  <div class="lead col-md-10 mx-auto mb-4"><?php echo  __("There is no more ad space on this account.","premiumpress"); ?></div>
  
  <div class="lead col-md-10 mx-auto mb-4"><?php echo  __("We'll redirect you to edit your %s page now.","premiumpress"); ?></div>
...
}elseif( 1 == 3 && !is_admin() && _ppt(array('mem','membership_required_listing'))  == '1' && $CORE->USER("membership_active", $userdata->ID) == 0 ){ 

?>
<div class="col-md-8 mx-auto">
  <?php _ppt_template( 'ajax-modal-memberships' ); ?>
</div>
<?php
...
<?php 
	...
	// NEW CATEGORY BOX IN 10.8.9
	if(!is_admin() && !isset($_GET['eid']) && !isset($_GET['tax']) && !in_array(THEME_KEY, array("sp")) && in_array(_ppt(array('lst', 'default_catsbox' )), array("","1")) == '1'){  _ppt_template('framework/design/add/add-category-new' ); } ?>
  
 
 <?php _ppt_template('framework/design/add/add-details' ); 
...
<div class="col-md-4">
<div id="mainsubmissionfromrightside"></div>

 
  <div id="add-sidebar-content">
  
  
   <?php _ppt_template('framework/design/add/add-featuredimage' ); ?>
   
  	<div class="card ppt-forms mb-4 rounded-0">  
    <div class="card-body">
      <?php _ppt_template('framework/design/add/add-category' ); ?>
      <?php if( in_array(THEME_KEY, array("vt","ph","so"))){ ?>
...
<script>
jQuery(document).ready(function(){ 
	if(jQuery("#add-sidebar-content").html().length < 100){
		jQuery("#add-sidebar-content").hide();
	}
});
</script>
  ...
  <div class="<?php if(!is_admin()){ ?>mb-5 pb-5<?php } ?>">
    ...
    <button class="btn btn-primary btn-block btn-xl mt-5 font-weight-bold text-uppercase hide-mobile <?php if(!is_admin()){ ?>mb-5<?php } ?>" id="mainSaveBtn" onclick="VALIDATE_FORM_DATA();"><?php echo __("Save Changes","premiumpress"); ?></button>
 
 
 <div style=" <?php if(_ppt('footer_mobile_menu') == 1){ echo "bottom:58px;"; }else{ echo "bottom:0px;"; } ?>" class="p-3 bg-primary show-mobile" id="mobile-save-listings">
  <div class="row">
    <div class="col-12 position-relative">
 
      <a href="javascript:void(0);" class="btn btn-system btn-block font-weight-bold" onclick="jQuery('#mainSaveBtn').trigger('click')"><?php echo __("Save Changes","premiumpress"); ?></a>
    </div>
  </div>
</div>
  ...
  // SHOW BIDDING DATA
		...
		?>
  <div class="small mt-4" style="max-height:400px; overflow-y: scroll;"> <?php echo str_replace("card","",$data['data']); ?> </div>
  <script>
...
jQuery(document).ready(function(){ 
   ...
   // PACKAGE LINK USED 
   if(isset($_GET['packageid']) && is_numeric($_GET['packageid']) && !isset($_GET['eid']) ){  
   ?>
    processPackage(<?php echo $_GET['packageid']; ?>);
   <?php
...
function updatePackageID(id){
	...
	if(HasShownConfirmMsg == 1 || confirm("<?php echo __("Are you sure you want to change your package?","premiumpress"); ?>")) {
	...
	// CHECK SPACE LEFT
	jQuery('.updatespaceleft').each(function () {		  
...
function VALIDATE_FORM_DATA(){ 
	...
	// SWITCH TAB	
	if(jQuery("#d1 .required-active").length == 0){
	jQuery('#tab2 a').trigger('click');
	}
...
function processPackage(mid){
	...
	if (typeof package[mid] !== "undefined") {
		...
		// PHOTOS & VIDES
		jQuery("#_ppt_images_counter .badge-success").html(package[mid]['images']);
  		jQuery("#_ppt_videos_counter .badge-success").html(package[mid]['videos']);
		
		// RECALCULATE IMAGE SPACE
		if(jQuery("#_ppt_images_counter").length > 0){
		_spaceLeft();
		}
		
		// RECALCULATE VIDEO SPACE
		if(jQuery("#_ppt_videos_counter").length > 0){
		_spaceVideoLeft();
		}
