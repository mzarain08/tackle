...
add_filter( 'ppt_blocks_args', 	array('block_category102',  'data') );
add_action( 'category102',  		array('block_category102', 'output' ) );
add_action( 'category102-css',  	array('block_category102', 'css' ) );
add_action( 'category102-js',  	array('block_category102', 'js' ) );

class block_category102 {
	...
	public static function data($a){ 
		...
		// 2. HOME DESIGNS		
		 	...
		 	$settings =  $CORE->LAYOUT("get_block_settings_defaults_new", array("category", "category102" ) );
		 	foreach($df as $h => $j){
	...
	foreach ($termdata as $term) {
			...
			if(isset($df['cat_subs']) && $df['cat_subs'] == "1"){
				...
				foreach ($termdatasub as $terms) {
					...
					"count"	=> $terms->count,  
					"image" => do_shortcode('[CATEGORYIMAGE term_id="'.$terms->term_id.'" pathonly=1 placeholder=1 big=1 tax="listing"]'),
...
echo ppt_theme_block_output($output, $df, array("category", "category102"));
...
.card-category102{position:relative;text-align:center;margin-bottom:30px;cursor:pointer;z-index:1;width:100%;height:230px; border-radius: 8px;     overflow: hidden; }
.card-category102:hover h5{bottom:42px;}
.card-category102:hover p{visibility:visible;bottom:15px;opacity:1;}
.card-category102 h5{position:absolute;bottom:30px;left:50%;z-index:1;width:100%;font-weight:500;text-align:center;text-transform:capitalize;-webkit-transform:translateX(-50%);transform:translateX(-50%);transition:all linear .3s;-webkit-transition:all linear .3s;-moz-transition:all linear .3s;-ms-transition:all linear .3s;-o-transition:all linear .3s;}
.card-category102 h5 a{color:#fff;}
