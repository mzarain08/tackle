...
add_filter( 'ppt_blocks_args', 	array('block_text156',  'data') );
add_action( 'text156',  		array('block_text156', 'output' ) );
add_action( 'text156-css',  	array('block_text156', 'css' ) );
add_action( 'text156-js',  	array('block_text156', 'js' ) );
...
<section class="section-60 ppt-block-text text156">
...
<?php
		...
		ob_end_clean();
echo ppt_theme_block_output($output, $df, array("text", "text156"));
...
.text156 .card-header .btn { font-size:16px; }
</style>
<?php
		$output = ob_get_contents();
		ob_end_clean();
