...
add_filter( 'ppt_blocks_args', 	array('block_text160',  'data') );
add_action( 'text160',  		array('block_text160', 'output' ) );
add_action( 'text160-css',  	array('block_text160', 'css' ) );
add_action( 'text160-js',  	array('block_text160', 'js' ) );
...
class block_text160 {
	...
	public static function data($a){  global $CORE;
		...
		// 2. HOME DESIGNS		
		 	...
		 	$settings =  $CORE->LAYOUT("get_block_settings_defaults_new", array("text", "text160" ) );
		 	foreach($df as $h => $j){
...
ob_end_clean();
echo ppt_theme_block_output($output, $df, array("text", "text160"));
