...
add_filter( 'ppt_blocks_args', 	array('block_text127',  'data') );
add_action( 'text127',  		array('block_text127', 'output' ) );
add_action( 'text127-css',  	array('block_text127', 'css' ) );
add_action( 'text127-js',  	array('block_text127', 'js' ) );
...
class block_text127 {
	...
	public static function data($a){  global $CORE;
		...
		return $a;
		 ...
		 $df = array(
		 	"image1" => DEMO_IMGS."?fw=text124g&t=".THEME_KEY,
	...
	$layout = array(
		...
		2 => "col-sm-6 col-md-6 col-lg-3",
		3 => "col-sm-6 col-md-6 col-lg-3",
		4 => "col-sm-6 col-md-6 col-lg-2",
  ...
  <div class="container">
    ...
    <?php $i=1; foreach($data as $cat){ ?>
         <div class="mb-4 <?php echo $layout[$i]; ?>">
            <div class="overflow-hidden text127-card style3 <?php echo $cat['css']; ?>">
...
ob_end_clean();
echo ppt_theme_block_output($output, $text_settings, array("text", "text127"));
...
.text127-card figure{margin:0}
.text127-card img{transition:all linear .25s}
.text127-card{background-color:#fff;box-shadow:0 0 25px rgba(0,0,0,.1);background-position:center;overflow:hidden;position:relative; transition:all linear .25s; width: 100%; border-radius: 10px; height:320px; min-width:200px;  }
.text127-card h4{ z-index: 2; position:absolute;font-size:25px;font-weight:700;color:#fff;transition:all linear .25s;text-shadow:0 0 20px rgba(0,0,0,.5)}
.text127-card h4 strong { font-weight:600; } 
 
.text127-card.style3 h4{left:0;bottom:20px;text-align:center;width:100%;font-size:30px;color:#fff; line-height:40px; }
.text127-card.style3 h4 span{font-size:13px;display:block}.text127-card.style3:hover img{filter:grayscale(100%)}
.text127-card.style3:hover h4{bottom:30px}   
