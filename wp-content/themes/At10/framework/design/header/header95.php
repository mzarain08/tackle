...
add_filter( 'ppt_blocks_args', 	array('block_header95',  'data') );
add_action( 'header95',  		array('block_header95', 'output' ) );
add_action( 'header95-css',  	array('block_header95', 'css' ) );
add_action( 'header95-js',  	array('block_header95', 'js' ) );

class block_header95 {
	...
	public static function data($a){ 
		...
		// DEFAULTS
		 ...
		 if(is_array($header_settings) && !empty($header_settings)){ 
			...
			$default_settings = $CORE->LAYOUT("get_block_settings_defaults_new", array("header", "header95" ) );	
...
echo str_replace("%logo%",$logoCode,str_replace("%topmenu%",$topMenuCode, ppt_theme_block_output($output, $df, array("header", "header95"))));
