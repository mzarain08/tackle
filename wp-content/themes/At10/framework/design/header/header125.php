...
add_filter( 'ppt_blocks_args', 	array('block_header125',  'data') );
add_action( 'header125',  		array('block_header125', 'output' ) );
add_action( 'header125-css',  	array('block_header125', 'css' ) );
add_action( 'header125-js',  	array('block_header125', 'js' ) );

class block_header125 {
	...
	public static function data($a){ 
		...
		// DEFAULTS
		 $cc = array(
			...
			"topmenu_bg_color" 		=> "#3b2546",
			"header_bg_color" 		=> "#52435a",
		 ...
		 if(is_array($header_settings) && !empty($header_settings)){ 
			...
			$default_settings = $CORE->LAYOUT("get_block_settings_defaults_new", array("header", "header125" ) );	
...
echo str_replace("%logo%",$logoCode,str_replace("%topmenu%",$topMenuCode, ppt_theme_block_output($output, $df, array("header", "header125"))));
