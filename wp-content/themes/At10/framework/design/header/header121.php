...
add_filter( 'ppt_blocks_args', 	array('block_header121',  'data') );
add_action( 'header121',  		array('block_header121', 'output' ) );
add_action( 'header121-css',  	array('block_header121', 'css' ) );
add_action( 'header121-js',  	array('block_header121', 'js' ) );

class block_header121 {
	...
	public static function data($a){ 
		...
		// DEFAULTS
		 ...
		 if(is_array($header_settings) && !empty($header_settings)){ 
			...
			$default_settings = $CORE->LAYOUT("get_block_settings_defaults_new", array("header", "header121" ) );	
...
echo str_replace("%logo%",$logoCode,str_replace("%topmenu%",$topMenuCode, ppt_theme_block_output($output, $df, array("header", "header121"))));
