...
add_filter( 'ppt_blocks_args', 	array('block_header96',  'data') );
add_action( 'header96',  		array('block_header96', 'output' ) );
add_action( 'header96-css',  	array('block_header96', 'css' ) );
add_action( 'header96-js',  	array('block_header96', 'js' ) );

class block_header96 {
	...
	public static function data($a){ 
		...
		// DEFAULTS
		 ...
		 if(is_array($header_settings) && !empty($header_settings)){ 
			...
			$default_settings = $CORE->LAYOUT("get_block_settings_defaults_new", array("header", "header96" ) );	
...
echo str_replace("%logo%",$logoCode,str_replace("%topmenu%",$topMenuCode, ppt_theme_block_output($output, $df, array("header", "header96"))));
