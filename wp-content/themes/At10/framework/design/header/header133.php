...
add_filter( 'ppt_blocks_args', 	array('block_header133',  'data') );
add_action( 'header133',  		array('block_header133', 'output' ) );
add_action( 'header133-css',  	array('block_header133', 'css' ) );
add_action( 'header133-js',  	array('block_header133', 'js' ) );

class block_header133 {
	...
	public static function data($a){ 
		...
		// DEFAULTS
		 ...
		 if(is_array($header_settings) && !empty($header_settings)){ 
			...
			$default_settings = $CORE->LAYOUT("get_block_settings_defaults_new", array("header", "header133" ) );	
...
echo str_replace("%logo%",$logoCode,str_replace("%topmenu%",$topMenuCode, ppt_theme_block_output($output, $df, array("header", "header133"))));
