...
add_filter( 'ppt_blocks_args', 	array('block_header119',  'data') );
add_action( 'header119',  		array('block_header119', 'output' ) );
add_action( 'header119-css',  	array('block_header119', 'css' ) );
add_action( 'header119-js',  	array('block_header119', 'js' ) );

class block_header119 {
	...
	public static function data($a){ 
		...
		// DEFAULTS
		 ...
		 if(is_array($header_settings) && !empty($header_settings)){ 
			...
			$default_settings = $CORE->LAYOUT("get_block_settings_defaults_new", array("header", "header119" ) );	
...
echo str_replace("%logo%",$logoCode,str_replace("%topmenu%",$topMenuCode, ppt_theme_block_output($output, $df, array("header", "header119"))));
