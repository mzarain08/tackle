...
add_filter( 'ppt_blocks_args', 	array('block_header120',  'data') );
add_action( 'header120',  		array('block_header120', 'output' ) );
add_action( 'header120-css',  	array('block_header120', 'css' ) );
add_action( 'header120-js',  	array('block_header120', 'js' ) );

class block_header120 {
	...
	public static function data($a){ 
		...
		// DEFAULTS
		 ...
		 if(is_array($header_settings) && !empty($header_settings)){ 
			...
			$default_settings = $CORE->LAYOUT("get_block_settings_defaults_new", array("header", "header120" ) );	
...
echo str_replace("%logo%",$logoCode,str_replace("%topmenu%",$topMenuCode, ppt_theme_block_output($output, $df, array("header", "header120"))));
