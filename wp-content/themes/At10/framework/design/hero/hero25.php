...
add_filter( 'ppt_blocks_args',  array('block_hero25',  'data') );
add_action( 'hero25',  			array('block_hero25', 'output' ) );
add_action( 'hero25-css',  		array('block_hero25', 'css' ) );
add_action( 'hero25-js',  		array('block_hero25', 'js' ) );
...
class block_hero25 {
	...
	public static function data($a){ 
		...
		$df = ppt_theme_blocks_defaults("hero"); 
		 ...
		 $cc = array(
		 	"image" => "https://premiumpress1063.b-cdn.net/_demoimagesv10/icons/bg8.png",
		...
		// 2. HOME DESIGNS		
		 	...
		 	$settings =  $CORE->LAYOUT("get_block_settings_defaults_new", array("hero", "hero25" ) );
		 	foreach($df as $h => $j){
...
<div class="col-lg-6">
    ...
    <h1 class="text-600 mb-4 fs-xxl lh-90" data-ppt-title><?php echo $df['title']; ?></h1>
...
ob_end_clean();
echo ppt_theme_block_output($output, $df, array("hero", "hero25"));
...
[data-ppt-blockid="hero25"] img { max-width:100%; } 
}
</style>
        <?php
