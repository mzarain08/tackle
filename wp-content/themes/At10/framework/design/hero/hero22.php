...
add_filter( 'ppt_blocks_args',  array('block_hero22',  'data') );
add_action( 'hero22',  			array('block_hero22', 'output' ) );
add_action( 'hero22-css',  		array('block_hero22', 'css' ) );
add_action( 'hero22-js',  		array('block_hero22', 'js' ) );
	...
	public static function data($a){ 
		...
		$df = ppt_theme_blocks_defaults("hero"); 
		 ...
		 $cc = array(
			...
			"image1" => "https://premiumpress1063.b-cdn.net/_demoimagesv10/icons/taxi.png",
			"btn_show" => 1,
			"btn2_show" => 0,	
			"searchbox" => 0,				
			"image1_show" => 1,
			...
			"title" 	=> "Build {awesome} websites today!",
		...
		// 2. HOME DESIGNS		
		 	...
		 	$settings =  $CORE->LAYOUT("get_block_settings_defaults_new", array("hero", "hero22" ) );
		 	foreach($df as $h => $j){
...
<header class="elementor_header header7 b-bottom ppt-fixed-header navbar-light border-bottom-white-50" >
...
<div class="col-lg-5">
 ...
 <div class="text-500 lh-40  fs-6" data-ppt-desc><?php echo $df['desc']; ?></div>
      ...
      <div class="<?php if(isset($df['image1_offset']) && $df['image1_offset'] == "1"){ ?>col-lg-5 offset-lg-1<?php }elseif(isset($df['image1_offset']) && $df['image1_offset'] == "2"){?>col-lg-4 offset-lg-2<?php }else{ ?>col-lg-7<?php } ?>">
        <div class="position-relative">
          ...
          <?php } ?>
          <img src="<?php echo $df['image1']; ?>" class=" <?php if(isset($df['image1_shadow'])){ echo $df['image1_shadow']; } ?>" alt="image" data-ppt-image1> </a>
        </div>
      </div>
      <?php } ?>
...
ob_end_clean();
echo ppt_theme_block_output($output, $df, array("hero", "hero22"));
...
[data-ppt-blockid="hero22"] img { max-width:100%; } 
}
</style>
        <?php
