...
add_filter( 'ppt_blocks_args',  array('block_hero23',  'data') );
add_action( 'hero23',  			array('block_hero23', 'output' ) );
add_action( 'hero23-css',  		array('block_hero23', 'css' ) );
add_action( 'hero23-js',  		array('block_hero23', 'js' ) );
...
class block_hero23 {
	...
	public static function data($a){ 
		...
		$df = ppt_theme_blocks_defaults("hero"); 
		 ...
		 $cc = array(
		 	"image" => "https://premiumpress1063.b-cdn.net/_demoimagesv10/icons/bg6.png",
		...
		// 2. HOME DESIGNS		
		 	...
		 	$settings =  $CORE->LAYOUT("get_block_settings_defaults_new", array("hero", "hero23" ) );
		 	foreach($df as $h => $j){
...
<div class="col-lg-5">
 ...
 <div class="text-500 lh-40  fs-6 opacity-5 mb-4" data-ppt-desc><?php echo $df['desc']; ?></div>
...
ob_end_clean();
echo ppt_theme_block_output($output, $df, array("hero", "hero23"));
...
[data-ppt-blockid="hero23"] img { max-width:100%; } 
}
</style>
        <?php
