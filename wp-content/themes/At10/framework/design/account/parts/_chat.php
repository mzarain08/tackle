...
<div class="card card-body mt-4 mb-4">
  ...
  <div style="    height: 450px;    overflow: auto;  overflow-y: scroll;" id="<?php echo str_replace("#","",$chat_div_id); ?>"></div>
  <hr />
...
jQuery(document).ready(function(){
  ...
  <a href="javascript:void(0);" onclick="ppt_chat_send_<?php echo $settings['pid']; ?>();" data-ppt-btn class=" btn-primary btn-sm  mr-3"><span><?php echo __("Send Message","premiumpress") ?></span></a> 
  
  
  <a href="javascript:void(0);" onclick="jQuery('.fileupload-buttonbar').toggle();" data-ppt-btn  class="btn btn-secondary btn-sm "><span><?php echo __("Attach File","premiumpress"); ?></span></a> </div>
</div>

<input type="hidden" id="ajax_chat_logs_<?php echo $settings['pid']; ?>" />
<script>
...
jQuery.ajax({
        ...
        success: function(response) {
			...
			if(response.status == "noupdate"){
				...
				jQuery('<?php echo $chat_div_id; ?>').html("<div class='col-12 text-center mt-5 pt-5  h4'><i class='fal fa-comments fa-4x mb-4 btn-block'></i><?php echo __("You have no chat history.","premiumpress") ?></div>");	
