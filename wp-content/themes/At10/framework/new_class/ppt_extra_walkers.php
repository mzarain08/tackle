	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( ! empty( $icon_class_string ) ) {
				...
				if(substr($icon_class_string,0,4) == "svg-"){
					global $CORE_UI;
					$svg =  substr($icon_class_string,4,1000);
															 
					if(isset($CORE_UI->icons_svg[$svg])){
					
						$icon_html = '<span ppt-icon-size="24" class="mr-2">'.$CORE_UI->icons_svg[$svg]."</span>";
					} 
				 
				}else{
				$icon_html = '<div ppt-icon-size="24"><span class="' . esc_attr( $icon_class_string ) . '" aria-hidden="true"></span></div>';
				}
			...
			if($item->menu_item_parent > 0 && strlen($item->description) > 1){
			
				$desc = "<div class='dropdown-desc'>".$item->description."</div>";
				
				$menu_title = "<div class='dropdown-wrap'><div class='dropdown-title d-flex'>".$icon_html."<div><span>".$title."</span></div> </div>" .$desc."</div>";
			
			}else{
			
			$menu_title = "<div class='d-inline-flex'>".$icon_html."<div><span>".$title."</span></div> </div>";
			...
			$item_output .= isset( $args->link_before ) ? $args->link_before . $menu_title  . $args->link_after : '';
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				$fallback_output .= '>';
				$fallback_output .= '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="' . esc_attr__( 'Add a menu', 'premiumpress' ) . '">' . esc_html__( 'Add a menu', 'premiumpress' ) . '</a></li>';
				$fallback_output .= '</ul>';
				if ( $container ) {
					$fallback_output .= '</' . esc_attr( $container ) . '>';
				}
		...
		private function separate_linkmods_and_icons_from_classes( $classes, &$linkmod_classes, &$icon_classes, $depth ) {
			...
			foreach ( $classes as $key => $class ) {
				...
				if ( preg_match( '/^disabled|^sr-only/i', $class ) ) {
					...
					$icon_classes[] = $class;
					unset( $classes[ $key ] );
				} elseif ( preg_match( '/^svg-(\S*)?|^svg(\s?)$/i', $class ) ) {
					// Glyphicons.
					$icon_classes[] = $class;
					unset( $classes[ $key ] );
				}
...
function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) { global $CORE;
		...
		if(isset($GLOBALS['CORE_THEME']['category_icon_small_'.$category->term_id]) && strlen($GLOBALS['CORE_THEME']['category_icon_small_'.$category->term_id]) > 2){		
			$icon = "<span class='fa text-primary ".$GLOBALS['CORE_THEME']['category_icon_small_'.$category->term_id]."'></span> ";		
		}
...
class ppt_comment_walker extends Walker_Comment {
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			<ol class="child-comments comments-list list-unstyled ml-md-5">
		...
		function start_el( &$output, $comment, $depth = 0, $args = array(), $id = 0 ) {
			...
			if(isset($GLOBALS['global_design4'])){
			_ppt_template('content-comment-new');
			}else{ 
...
class Walker_CategorySelectionBAK extends Walker_Category {  
     ...
     function start_el(&$output, $item, $depth=0, $args=array(), $id = 0) { global $CORE; 
		...
		if(isset($GLOBALS['CORE_THEME']['category_icon_small_'.$item->term_id]) && strlen($GLOBALS['CORE_THEME']['category_icon_small_'.$item->term_id]) > 1){			
			$image = "<span class='fa ".$GLOBALS['CORE_THEME']['category_icon_small_'.$item->term_id]."'></span>"; 
		}		 
	...
	function start_lvl( &$output,  $depth = 0, $args = array(), $id = 0 ) { global $item;
		...
		onclick="jQuery(\'.hidesub'. $GLOBALS['thiscatitemid'].'\').hide(); jQuery(\'.showsub'. $GLOBALS['thiscatitemid'].'\').show(); jQuery(\'.subcats_'.$GLOBALS['thiscatitemid'].'\').hide();"> <span class="fa fa-chevron-up"></span> </a>';
...
class Bootstrap_Walker_Mobile extends Walker_Nav_Menu {     
        ...
        {    
            ...
            if ($item->hasChildren && $depth == 0 && !isset($GLOBALS['flasg_smalldevicemenubar']) ) { 
                ...
                $item_output .= '<a'. $attributes .' class="txt"><em>span class="fa fa-angle-right"></span> '.$icon; 
            }
	  		$iconpack = false;	
