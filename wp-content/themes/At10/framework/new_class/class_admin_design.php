...
function _custom_metabox(){ 
	...
	add_meta_box( 'box_blogop',"Show/Hide Default Header", array($this, '_box_blogtop' ), array('post'), 'side', 'high' );
...
function _custom_metabox_save($post_id){ global $pagename, $wpdb, $CORE;
 ...
 function _box_blogtop(){ global $post, $CORE; 
...
<p><?php echo __("Show blog image + social bar","premiumpress"); ?></p>
<select name="custom[blog_header]" style="width:100%;">
 
  <option value="yes" <?php if($blog_header == "yes"){  echo "selected=selected"; } ?>>Yes</option>
  <option value="no" <?php if($blog_header == "no"){  echo "selected=selected"; } ?>>No</option>
...
setTimeout(
  function(){
	...
	var pagetm = jQuery("#inspector-select-control-1").val();
	var myarray = [];
	myarray.push("templates/tpl-page-aboutus.php");	
	myarray.push("_coupon/tpl-page-stores.php");
	myarray.push("_dating/tpl-chatroom.php");
	myarray.push("templates/tpl-add.php");
	myarray.push("templates/tpl-callback.php");
	myarray.push("templates/tpl-page-aboutus.php");
	myarray.push("templates/tpl-page-account.php");
	myarray.push("templates/tpl-page-blog.php");
	myarray.push("templates/tpl-page-cart.php");
	myarray.push("templates/tpl-page-categories.php");
	myarray.push("templates/tpl-page-checkout.php");
	myarray.push("templates/tpl-page-contact.php");
	myarray.push("templates/tpl-page-faq.php");
	myarray.push("templates/tpl-page-how.php");
	myarray.push("templates/tpl-page-memberships.php");
	myarray.push("templates/tpl-page-privacy.php");
	myarray.push("templates/tpl-page-sellspace.php");
	myarray.push("templates/tpl-page-terms.php");
	myarray.push("templates/tpl-page-testimonials.php");
 	
	
	if(jQuery.inArray(pagetm, myarray) !== -1){
	
	text = pagetm.replace("_coupon", "").replace("_dating", "").replace("templates/", "").replace("tpl-", "").replace(".php", "");
	  
	jQuery("#elementor-switch-mode").html("<a class='button-primary button-large' href='admin.php?page=design&lefttab=pages&ep="+text+"'>Edit with Elementor</a>");
	
	jQuery("#elementor-editor").html("<a class='button-primary button-large' href='admin.php?page=design&lefttab=pages&ep="+text+"'>Edit with Elementor</a>");
 ...
 case "upload": { 
      ...
      <div class="input-group mb-2">
        <input type="text" name="admin_values[<?php echo $dbkey; ?>][<?php echo $key; ?>]" value="<?php 
			   ...
			   if( _ppt(array($dbkey, $key)) == "" && isset($item['d']) && $item['d'] !=""){ 
