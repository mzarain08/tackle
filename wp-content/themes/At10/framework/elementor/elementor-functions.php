...
function _ppt_elementor_get_blocks($cat, $widget){ global $CORE;

	$list = array();
	$gs = $CORE->LAYOUT("load_all_by_cat",  $cat);
	foreach($gs as $k => $g){ 		
				
		if(isset($g['widget']) && is_array($widget) && in_array($g['widget'], $widget ) ){	
		
			$list[$k] = $g['name'];	
			
		}elseif(isset($g['widget']) && $g['widget'] == $widget){	
