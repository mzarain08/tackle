...
class auctiontheme  {
    ...
    function __construct(){ global $CORE;
        ...
        add_shortcode( 'MILES', array($this,'shortcode_miles') );
        add_shortcode( 'MAKE', array($this,'shortcode_make') );
        add_shortcode( 'MODEL', array($this,'shortcode_model') );


    }


    function shortcode_year( $atts, $content = null ) { global $userdata, $CORE, $post;

        // GET LIST
        $LIST = get_post_meta($post->ID,"year", true);
        if(is_numeric($LIST)){
            return $LIST;
        }else{
            return "n/a";
    ...
    function shortcode_miles( $atts, $content = null ) { global $userdata, $CORE, $post;

        // GET LIST
        $LIST = get_post_meta($post->ID,"miles", true);
        if(is_numeric($LIST)){
            return number_format($LIST);
        }else{
            return "n/a";
        }
    ...
    function shortcode_model( $atts, $content = null ) { global $CORE, $post;

        // EXTRACT OPTIONS
        extract( shortcode_atts( array( 'postid' => '', 'link' => false ), $atts ) );
        ...
        // GET LIST
        $val = get_post_meta($postid,"model", true);

        // RETURN
        if($link){
            return "<a href='".home_url()."/?s=&model=".$val."'>".$val."</a>";
        }else{
            return $val;
        }
    ...
    function shortcode_make( $atts, $content = null ) { global $CORE, $post;
        ...
        // GET LIST
        $val = get_post_meta($postid,"make", true);
        if(isset($GLOBALS['ppt_makes'])){

            $list = $GLOBALS['ppt_makes'];
            $data = array();
            foreach($list as $m){
                if($val == str_replace(" ","",strtolower($m))){
                    $val = $m;
                }
            }
        }
        ...
        // RETURN
        if($link){
            return "<a href='".home_url()."/?s=&make=".str_replace(" ","",strtolower($val))."'>".$val."</a>";
        }else{
            return $val;
        }
    ...
    function _user_types(){
        ...
        $accountTypes = array(
            ...
            "user_fr" => array(
                "name" 		=> __("Buyer","premiumpress"),
                "desc" 		=> "",
                "img" 		=> DEMO_IMG_PATH."_usertypes/at1.jpg",
                "can_add"	=> 0,
            ),

            "user_em" => array(
                "name" 		=> __("Seller","premiumpress"),
                "desc" 		=> "",
                "img" 		=> DEMO_IMG_PATH."_usertypes/at2.jpg",
                "can_add"	=> 1,
            ),
    ...
    function _hook_register_top(){ global $CORE;
        ...
        <script>
            ...
            function switchtypedata(thisid){
                ...
                <?php } ?>
                jQuery("#regtopmsg").html("<h4>"+names[thisid]+"</h4>");
        ...
        <input name="user_type" id="user_type" type="hidden" value="user_fr" />
        <?php
    ...
    */
    ...
    function account_status_show($postid){ global $CORE, $CORE_AUCTION;
        ...
        //1. GET EXPIRY DATE
        $vv = $CORE->date_timediff( get_post_meta($postid,'listing_expiry_date',true) );
        if($vv['expired'] == 1){
    ...
    */
    function shortcode_auction_timer( $atts, $content = null ) { global $userdata, $CORE_AUCTION, $wpdb, $CORE, $post;
      ...
      <span
              data-ppt-countdown="<?php echo $expiry_date; ?>"
    ...
    */
    function _hook_customfields($c){ global $CORE;
        ...
        if($canEditPrice){
            $c[$o]['title'] 	= __("Buy It Now Price","premiumpress");
            $c[$o]['name'] 		= "price_bin";
            $c[$o]['type'] 		= "price";
            $c[$o]['class'] 	= "form-control";
        ...
        if($canEditPrice){
            $c[$o]['title'] 	= __("Buy It Now Qty Available","premiumpress");
            $c[$o]['name'] 		= "qty";
            $c[$o]['type'] 		= "text";
            $c[$o]['class'] 	= "form-control";
            $c[$o]['help'] 		= __("Here you can set a qty available for buyers to purchase.","premiumpress");
            //$c[$o]['required'] 	= true;
            $c[$o]['defaultvalue'] 	= "1";
            $o++;
        }
    ...
    */
    function bidding_history($postid, $jason = false){
        ...
        if(is_array($hbid) && isset($hbid['userid']) ){
            ...
            $hname = $CORE->USER("get_display_name", $hbid['userid']);
            ...
            <?php }else{ ?>
                ...
                <?php $c=1; foreach($bidding_history as $date => $bhistory){  ?>
                    <div class="row small">
                        ...
                        <div class="col-5">
                            <?php /*<a href="<?php echo get_author_posts_url( $bhistory['userid'] ); ?>">*/ ?>
                            <?php echo $bhistory['user']; ?>
                        </div>
    ...
    function _end_auction($postid){ global $CORE, $post; $item_status = "";
        ...
        if($hbid['user'] == "nobidders"){
            ...
            if( $qty > 0 ){
                ...
                update_post_meta($postid,'qty-sold-auction', get_post_meta($postid,'qty-sold', true) + $qty );

            }

            if( $qty > 0 && ( strtotime($expires) > strtotime(current_time( 'mysql' )) ) ){
    ...
    */
    ...
    function _relist_auction($postid, $force = false){ global $CORE, $wpdb;
        ...
        $expiry_date = get_post_meta($postid,'listing_expiry_date',true);
        $ff = $CORE->date_timediff($expiry_date,'');
    ...
    */
                ...
                if(is_array($history) && !empty($history)){
                    ...
                    foreach($history as $maxbid => $data){
                        ...
                        if($i == 1 && $data['amount'] > 0 ){ // this is the current highest bidder
                            ...
                            if($data['userid'] != $WINNERID && is_numeric($data['userid']) && !in_array($data['userid'],$sent_to_array)){
    ...
    function _add_paymentform($postid, $winnerid){ global $wpdb, $CORE, $userdata;
        ...
        foreach($tt as $t){
            ...
            if($winnerid == get_post_meta($t->ID,'buyer_id',true)){
                ...
                $user_bidding_data = get_user_meta($winnerid,'user_bidding_data',true);
                if(!is_array($user_bidding_data)){ $user_bidding_data = array(); }

                $offerid = "";
                if(isset($user_bidding_data["offer_id"])){
                    $offerid =  $user_bidding_data["offer_id"];
                }
    ...
    function _add_user_bidhistory($postid, $userid, $price, $type){ global $userdata, $CORE, $post, $wpdb;
        ...
        $user_bidding_data[] = array('postid' => $postid, 'date' => current_time( 'mysql' ), 'bid_type' => $type, "amount" => $price, "offer_id" => $POSTID );
    ...
    /*
        ...
        to the auction
    */
    function _add_bidhistory($postid = 0, $userid = 0, $amount, $type = "buynow"){ global $CORE;
        ...
        $history[strtotime(current_time( 'mysql' ))] = array( 'amount' => $amount, 'userid' => $userid, 'user' => $CORE->USER("get_display_name", $userid), 'bid_type' => $type, "date" => current_time( 'mysql' )   );
    ...
    */
    function _add_bid($postid, $userid, $amount, $ic ="add", $type = ""){ global $CORE, $post, $userdata;
        ...
        $offerID = $this->_add_user_bidhistory($postid, $userid, $amount, $type);
    ...
    function _wp_head(){ global $CORE, $post, $userdata;
        ...
        if(isset($_POST['auction_action'])){
            ...
            switch($_POST['auction_action']){
                ...
                case "buynow": {
                    $buyqty = $_REQUEST['buyqty'];
                    $pricetotalBuy = $_REQUEST['pricetotal'];
                    ...
                    update_post_meta($post->ID,'price_current', hook_price_save($pricetotalBuy));

                    // ADD HISTORY
                    //$this->_add_bid($post->ID, $userdata->ID, $bin_price, "final", "buynow");
                    $this->_add_bid($post->ID, $userdata->ID, $pricetotalBuy, "final", "buynow");
                    ...
                    if( $qty > 0 ){
                        ...
                        if(isset($buynowdata[$userdata->ID])){
                            //$buynowdata[$userdata->ID] = array("price" => $bin_price, "date" => current_time( 'mysql' ), "qty" => $buynowdata[$userdata->ID]['qty'] + $buyqty, "username" =>  $CORE->USER("get_display_name", $userdata->ID) );
                            $buynowdata[$userdata->ID] = array("price" => $pricetotalBuy, "date" => current_time( 'mysql' ), "qty" => $buynowdata[$userdata->ID]['qty'] + $buyqty, "username" =>  $CORE->USER("get_display_name", $userdata->ID) );
                        }else{
                            //$buynowdata[$userdata->ID] = array("price" => $bin_price, "date" => current_time( 'mysql' ), "qty" => $buyqty, "username" =>  $CORE->USER("get_display_name", $userdata->ID) );
                            $buynowdata[$userdata->ID] = array("price" => $pricetotalBuy, "date" => current_time( 'mysql' ), "qty" => $buyqty, "username" =>  $CORE->USER("get_display_name", $userdata->ID) );
                        }

                        update_post_meta($post->ID,'user_buynow_data', $buynowdata);

                        // REDUCE QTY BY ONE
                        update_post_meta($post->ID,'qty', get_post_meta($post->ID,'qty', true) - $buyqty );
                        update_post_meta($postid,'qty-sold-buynow', get_post_meta($postid,'qty-sold-buynow', true) + $buyqty );
            ...
            switch($_POST['auction_action']){
                    ...
                    switch($_POST['type']){
                        ...
                        case "penny": {
                            ...
                            if($c > 0){
                                ...
                                //2. UPDATE TIMER
                                $expiry_date = get_post_meta($_POST['pid'],'listing_expiry_date',true);
                                $expiry_date = date("Y-m-d H:i:s", strtotime( $expiry_date . " +10 seconds") );
                                update_post_meta($_POST['pid'],'listing_expiry_date', $expiry_date );
                ...
                case "buybox_load": {
                    ...
                    if(is_numeric($_POST['pid'])){
                        ...
                        $arr = array('price' => $price_current, "date" => $expiry_date, "credit" => $credit, "status" => $status );
